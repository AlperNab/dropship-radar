...
**Domain:** `E-commerce / Product Research` · **Suite:** `E-commerce Growth Suite` · **Accent:** `#a3e635`
...
**Dropship Radar** is a standalone, browser-based AI workflow app for **E-commerce / Product Research**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `niche_scan`<br>Niche scan | text | Yes | — | Affects input: Niche scan. |
...
| `target_market`<br>target market | text | No | — | Affects customization: target market. |
| `niche`<br>niche | text | No | — | Affects customization: niche. |
| `budget`<br>budget | text | No | — | Affects customization: budget. |
| `supplier_region`<br>supplier region | text | No | — | Affects customization: supplier region. |
| `shipping_limits`<br>shipping limits | text | No | — | Affects customization: shipping limits. |
| `ad_platform`<br>ad platform | select | No | Meta Ads | Affects customization: ad platform. |
| `banned_categories`<br>banned categories | text | No | — | Affects customization: banned categories. |
| `margin_target`<br>margin target | text | No | — | Affects customization: margin target. |
...
- **ad platform**: Meta Ads, Google Ads, TikTok, Instagram, LinkedIn, YouTube, Amazon, Shopify, Web
...
uvicorn app.main:app --host 127.0.0.1 --port 9117
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9117`. |
...
- Export download
- Job history
- Marketplace/supplier/ad library APIs optional
...
```bash
curl http://127.0.0.1:9117/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9117/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9117 uvicorn app.main:app --host 0.0.0.0 --port 9117
```
...
- `plugins/dropship-radar.json` for plugin metadata.
